...
Delivery outcomes give Arklow a useful view of capacity from the first action. Your existing metrics can widen that view. Scale targets add capacity when a protected backlog indicates sustained demand.

```mermaid actions={false}
flowchart LR
    S["Sources"] --> A["Durable actions"]
    A --> R["Rules"]
    R --> G["Admission"]
    G --> D["Destinations"]
    D --> O["Outcomes"]
...
<Note>
  Arklow can also track work that happens externally. Use the [SDK](/resources/destinations/sdk/index) when moving existing pipelines behind Arklow is impractical.
</Note>
