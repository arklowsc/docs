...
Use **GCP managed instance group (manual)** when the group is resized directly. Arklow requests a concrete target size within the target's replica range.

Arklow discovers both modes from the managed instance group inventory. A group with an attached autoscaler uses the autoscaler-floor mode; a group without one uses desired-count mode.
