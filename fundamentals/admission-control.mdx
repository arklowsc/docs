...
Decisions made by admission control are done so quickly. For performance questions, our [SDK](/resources/destinations/sdk/index#performance) documentation describes what to expect.
...
| [Shared capacity](/resources/capacity-pools/index) | Available capacity derived from delivery evidence and connected infrastructure metrics, plus the lane's tightest allocation across its pools |
| Operator controls | Temporary caps or a pause applied to the lane |

## Running and unsettled capacity

In addition, Admission takes great care to measure two important observables:
...
A destination that acknowledges work in its response may use running capacity only briefly. A destination that accepts work for later settlement continues to occupy unsettled capacity after the response. For example, work given to one of your SDK listeners, will be counted as unsettled until the work is marked completed, either by you, or in the case of `Defer`, if you stop replying.
...
While the admission member of [Write Control](https://app.arklow.io/dashboard/settings?tab=organization) is off, admission decisions are recorded and every release is immediate. While it is on, the decisions on this page apply.
