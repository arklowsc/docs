...
- Maximizing the useful capacity of your infrastructure's resources.
...
We define `document.generate` as an [action definition](/resources/actions/index). This allows Arklow to have a way to refer to this body of work, without needing to know exactly what's being done.
...
Admission evaluates the selected lane and the destination's [capacity pools](/resources/capacity-pools/index). It applies the lane's tightest allocation across those pools.
...
When that supply is constrained, admission can hold a busy contributor while preserving room for other active lanes. [Capacity pools](/resources/capacity-pools/index) define membership, available capacity, and allocation.
...
<Columns cols={2}>
  ...
  >
    Shared supply, allocation, and attached scale targets.
  </Card>
</Columns>
